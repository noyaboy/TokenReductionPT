...
def dir_main():
    ...
    for index, row in df.iterrows():
        ...
        # 檢查是否有 deit, evit, vit 等不同模型，根據需要調整參數
        if "deit" in row["model"] or "evit" in row["model"]:
            args.viz_mode = False
        else:
            args.viz_mode = viz_mode

        # 處理新增的參數
        args.ifa_head = bool(row.get("ifa_head", False))
        args.clc = bool(row.get("clc", False))
        args.num_clr = int(row.get("num_clr", 0))
        ...
        if "dataset_name" in row:
            args.dataset_name = row["dataset_name"]
