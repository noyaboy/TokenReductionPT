...
__all__ = ['AMAWS']  # model_registry will add each entrypoint fn to this
...
class Block(nn.Module):
    def __init__(
            ...
            dropped_token_fusion: bool = True,
            fusion_crit_norm: bool = False,
            lr_dim: int = 16,
        ...
        self.dropped_token_fusion = dropped_token_fusion
        self.fusion_crit_norm = fusion_crit_norm

        self.curr_layer = curr_layer

        if keep_rate < 1:
            if self.dropped_token_fusion and lr_dim:
                lr_dim = lr_dim
                # self.compression_factor = compression_factor
                self.compression_factor = dim // lr_dim
                self.reducer = nn.Sequential(
                    Rearrange('b (cf nr) d -> b nr cf d', cf=self.compression_factor),
                    nn.LayerNorm(dim),
                    nn.Linear(dim, lr_dim),
                    Rearrange('b nr cf dc -> b nr (cf dc)')

                    # Rearrange('b (nr cf) d -> b nr (d cf)', cf=compression_factor),
                    # nn.Linear(dim * compression_factor, dim),

                    # Rearrange('b (nr cf) d -> b nr (d cf)', cf=compression_factor),
                    # nn.LayerNorm(dim * compression_factor),
                    # nn.Linear(dim * compression_factor, lr_dim),
                    # # nn.LayerNorm(lr_dim),
                    # # nn.GELU(),
                    # nn.Linear(lr_dim, dim),

                    # Rearrange('b (nr cf) d -> b d cf nr', cf=compression_factor),
                    # # nn.BatchNorm2d(dim),
                    # nn.Conv2d(dim, dim, (compression_factor, 1), (compression_factor, 1), groups=64),
                    # # nn.BatchNorm2d(dim),
                    # # nn.ReLU(inplace=True),
                    # Rearrange('b d 1 nr -> b nr d'),

                    # Rearrange('b (nr cf) d -> b cf d nr', cf=compression_factor),
                    # # # nn.BatchNorm2d(dim),
                    # nn.Conv2d(compression_factor, 1, (1, 1), (1, 1)),
                    # # # nn.BatchNorm2d(dim),
                    # # # nn.ReLU(inplace=True),
                    # Rearrange('b 1 d nr -> b nr d'),
                )

            # print(self.reducer)
            print(f'Layer {curr_layer+1} AMAWS reduction with dropped fusion: {dropped_token_fusion}, norm criteria: {fusion_crit_norm} and compression_factor: {self.compression_factor}')

        self.keep_rate = keep_rate
        assert 0 < keep_rate <= 1, "keep_rate must > 0 and <= 1, got {0}".format(keep_rate)

    def select_tokens_fuse_pruned(self, x, select_crit, left_tokens, N, C):
        _, idx = torch.topk(select_crit, left_tokens, dim=1, largest=True, sorted=True)  # [B, left_tokens]
        ...
        if self.dropped_token_fusion:
            ...
            # if self.fusion_crit_norm:
            #     select_crit_min_max =  ((select_crit - torch.min(select_crit, dim=-1, keepdim=True)[0]) 
            #                             / (torch.max(select_crit, dim=-1, keepdim=True)[0] - torch.min(select_crit, dim=-1, keepdim=True)[0]))
            #     non_topk_attn = torch.gather(select_crit_min_max, dim=1, index=compl)  # [B, N-1-left_tokens]
            # else:
            #     non_topk_attn = torch.gather(select_crit, dim=1, index=compl)  # [B, N-1-left_tokens]

            # extra_token = torch.sum(non_topk * non_topk_attn.unsqueeze(-1), dim=1, keepdim=True)  # [B, 1, C]

            num_comp_tokens = self.compression_factor * (non_topk.shape[1] // self.compression_factor)
            # num_comp_tokens = non_topk.shape[1] // self.compression_factor
            comp_tokens = non_topk[:, :num_comp_tokens]
            # print(num_comp_tokens, comp_tokens.shape, non_topk.shape, self.compression_factor)
            extra_token = self.reducer(comp_tokens)
            # print(non_topk.shape, num_comp_tokens, comp_tokens.shape, extra_token.shape, x_others.shape)
    ...
    def prune_tokens(self, x, attns):
        B, N, C = x.shape

        # tmp: x_cls and x_others
        # index: indexes correspond to top_k (left_tokens) attn tokens
        # based on select_crit repeated across C channels [b, left_tokens, C]
        # idx: indexes correspond to top_k (left_tokens) attn tokens based on select_crit [b, left_tokens]
        # select_crit: attn for cls token: [b, s-1]
        ...
        if self.keep_rate < 1:  # double check the keep rate
            ...
            if left_tokens != N - 1:
                ...
                select_crit = calc_amaws(attns, self.curr_layer)

                x, idx, compl = self.select_tokens_fuse_pruned(x, select_crit, left_tokens, N, C)
    ...
    def forward(self, x: torch.Tensor, attns=None) -> torch.Tensor:
        idx = None
        ...
        x, idx, compl = self.prune_tokens(x, attns)
...
class AMAWS(nn.Module):
    ...
    def __init__(
        ...
        # dropped_token_fusion = getattr(args, 'dropped_token_fusion', True)
        dropped_token_fusion = True
        # compression_factor = getattr(args, 'compression_factor', [100])
        lr_dim = getattr(args, 'lr_dim', [16])
        fusion_crit_norm = getattr(args, 'fusion_crit_norm', False)

        if len(token_ratio) == 1:
            token_ratio = [token_ratio[0] for _ in range(len(reduction_loc))]

        # if len(compression_factor) == 1:
        #     for i in range(len(reduction_loc) - 1):
        #         compression_factor.append(int(compression_factor[0] * token_ratio[i]))
        if len(lr_dim) == 1:
            for i in range(len(reduction_loc) - 1):
                lr_dim.append(int(lr_dim[i] * 2))
        ...
        # compression_factor_full = [1 for _ in range(depth)]
        # for idx, loc in enumerate(reduction_loc):
        #     compression_factor_full[loc] = compression_factor[idx]
        lr_dim_full = [1 for _ in range(depth)]
        for idx, loc in enumerate(reduction_loc):
            lr_dim_full[loc] = lr_dim[idx]
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                act_layer=act_layer,
                mlp_layer=mlp_layer,
                dropped_token_fusion=dropped_token_fusion,
                fusion_crit_norm=fusion_crit_norm,
                lr_dim=lr_dim_full[i],
                # compression_factor=compression_factor_full[i],
        ...
        remaining_n = self.embed_len
        self.remaining_tokens.append(remaining_n)
        for tr in token_ratio:
            remaining_n = math.ceil(tr * (remaining_n - 1))
            self.remaining_tokens.append(remaining_n)
        ...
        for name, _ in self.named_modules():
            if ('attn_drop' in name or 'pre_softmax' in name):
                layers.append(name)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            x, left_token, sample_idx, compl = blk(x, self.attn_dic)
...
@torch.no_grad()
def _load_weights(model: AMAWS, checkpoint_path: str, prefix: str = '') -> None:
...
default_cfgs = {
    ...
    'amaws_vit_base_patch16_224.orig_in21k': _cfg(
    ...
    'amaws_vit_small_patch16_224.dino': _cfg(
        url='https://dl.fbaipublicfiles.com/dino/dino_deitsmall16_pretrain/dino_deitsmall16_pretrain.pth',
        hf_hub_id='timm/vit_small_patch16_224.dino',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0),
    'amaws_vit_base_patch16_224.dino': _cfg(
    ...
    'amaws_vit_small_patch14_reg4_dinov2.lvd142m': _cfg(
        ...
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 518, 518), crop_pct=1.0),
    'amaws_vit_base_patch14_reg4_dinov2.lvd142m': _cfg(
    ...
    'amaws_flexivit_base.1200ep_in1k': _cfg(
        ...
        input_size=(3, 240, 240), crop_pct=0.95),
    'amaws_flexivit_large.1200ep_in1k': _cfg(
        ...
        input_size=(3, 240, 240), crop_pct=0.95),
    'amaws_flexivit_base.patch16_in21k': _cfg(
...
@register_model
def amaws_vit_tiny_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('amaws_vit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_small_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('amaws_vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_small_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/8)
    """
    model_args = dict(patch_size=8, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('amaws_vit_small_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('amaws_vit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/8) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=8, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('amaws_vit_base_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_large_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('amaws_vit_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_large_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('amaws_vit_large_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_huge_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Huge model (ViT-H/14) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_args = dict(patch_size=14, embed_dim=1280, depth=32, num_heads=16)
    model = _create_vision_transformer('amaws_vit_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch16_224_miil(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    Weights taken from: https://github.com/Alibaba-MIIL/ImageNet21K
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False)
    model = _create_vision_transformer(
        'amaws_vit_base_patch16_224_miil', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_medium_patch16_gap_240(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'amaws_vit_medium_patch16_gap_240', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_medium_patch16_gap_256(pretrained: bool = False, **kwargs):
    ...
    model_args = dict(
        ...
        global_pool='avg', qkv_bias=False, init_values=1e-6, fc_norm=False)
    model = _create_vision_transformer(
        'amaws_vit_medium_patch16_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'amaws_vit_base_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_huge_patch14_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'amaws_vit_huge_patch14_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_giant_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'amaws_vit_giant_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch16_clip_224(pretrained: bool = False, **kwargs):
    """ ViT-B/16 CLIP image tower
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
        'amaws_vit_base_patch16_clip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_flexivit_small(pretrained: bool = False, **kwargs):
    """ FlexiViT-Small
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, no_embed_class=True)
    model = _create_vision_transformer('amaws_flexivit_small', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_flexivit_base(pretrained: bool = False, **kwargs):
    """ FlexiViT-Base
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, no_embed_class=True)
    model = _create_vision_transformer('amaws_flexivit_base', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_flexivit_large(pretrained: bool = False, **kwargs):
    """ FlexiViT-Large
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, no_embed_class=True)
    model = _create_vision_transformer('amaws_flexivit_large', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_small_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-S/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=384, depth=12, num_heads=6, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'amaws_vit_small_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-B/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'amaws_vit_base_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_large_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'amaws_vit_large_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_giant_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    ...
    model = _create_vision_transformer(
        'amaws_vit_giant_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_small_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'amaws_vit_small_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'amaws_vit_base_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_large_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'amaws_vit_large_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_giant_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    # The hidden_features of SwiGLU is calculated by:
    # hidden_features = (int(hidden_features * 2 / 3) + 7) // 8 * 8
    ...
    model = _create_vision_transformer(
        'amaws_vit_giant_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch16_siglip_224(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'amaws_vit_base_patch16_siglip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'amaws_vit_base_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch16_siglip_512(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'amaws_vit_base_patch16_siglip_512', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_large_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'amaws_vit_large_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_medium_patch16_reg4_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8, class_token=True,
        no_embed_class=True, reg_tokens=4,
    )
    model = _create_vision_transformer(
        'amaws_vit_medium_patch16_reg4_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_medium_patch16_reg4_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8,
        class_token=False, no_embed_class=True, reg_tokens=4, global_pool='avg',
    )
    model = _create_vision_transformer(
        'amaws_vit_medium_patch16_reg4_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_vit_base_patch16_reg8_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False,
        no_embed_class=True, global_pool='avg', reg_tokens=8,
    )
    model = _create_vision_transformer(
        'amaws_vit_base_patch16_reg8_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_deit_tiny_patch16_224(pretrained=False, **kwargs):
    """ DeiT-tiny model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('amaws_deit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_deit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('amaws_deit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_deit_base_patch16_224(pretrained=False, **kwargs):
    """ DeiT base model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('amaws_deit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_deit3_small_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('amaws_deit3_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_deit3_medium_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('amaws_deit3_medium_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_deit3_base_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('amaws_deit3_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_deit3_large_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('amaws_deit3_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def amaws_deit3_huge_patch14_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('amaws_deit3_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
