...
from .attention_rollout import calc_cum_attention

__all__ = ['MCRiT']  # model_registry will add each entrypoint fn to this
...
class Block(nn.Module):
    def __init__(
            ...
            dropped_token_fusion: bool = False,
            fusion_crit_norm: bool = False,
            keep_rate: float = 1.0,
            select_crit: str = None,
            curr_layer: int = 0,
            # mlar_loc: bool = False,
            # mlar_method: str = 'cls',
            # rollout_method: str = 'dual',
    ) -> None:
        super().__init__()
        ...
        self.select_crit = select_crit
        self.curr_layer = curr_layer

        self.mlar = any([kw == select_crit for kw in ['dar', 'ar', 'dmaws', 'maws', 'glsdmaws', 'glsmaws', 'mix']])

        if self.select_crit:
             print(f'Layer {self.curr_layer+1} reducing with {self.select_crit}, with dropped fusion: {dropped_token_fusion} and norm criteria: {fusion_crit_norm}')
    ...
    def prune_tokens(self, x, aux=None):
        B, N, C = x.shape
        ...
        if self.keep_rate < 1:  # double check the keep rate
            # if self.training:
            #     eps = 0.01
            #     kr = random.uniform(self.keep_rate - eps, self.keep_rate + eps)
            #     left_tokens = math.ceil(kr * (N - 1))
            # else:
            ...
            if left_tokens != N - 1:
                ...
                if self.select_crit == 'agls':
                    g = x[:, :1]
                    l = x[:, 1:]
                    select_crit = F.cosine_similarity(g, l, dim=-1)

                    select_crit_2 = aux[:, :, 0, 1:]  # [B, H, N-1]
                    select_crit_2 = select_crit_2.mean(dim=1)  # [B, N-1]

                    select_crit = select_crit * select_crit_2

                    x, idx, compl = self.select_tokens_fuse_pruned(x, select_crit, left_tokens, N, C)

                elif self.select_crit == 'gls':
                    g = x[:, :1]
                    l = x[:, 1:]
                    select_crit = F.cosine_similarity(g, l, dim=-1)
                ...
                elif self.select_crit == 'attn':
                    select_crit = aux[:, :, 0, 1:]  # [B, H, N-1]
                    select_crit = select_crit.mean(dim=1)  # [B, N-1]

                    x, idx, compl = self.select_tokens_fuse_pruned(x, select_crit, left_tokens, N, C)

                elif self.mlar:
                    if self.select_crit == 'mix' and self.training:
                        crit = random.choice(['maws', 'dmaws', 'attn', 'gls'])
                        select_crit = calc_cum_attention(aux, self.curr_layer, crit, x)
                    elif self.select_crit == 'mix':
                        select_crit = calc_cum_attention(aux, self.curr_layer, 'dmaws', x)
                    else:
                        select_crit = calc_cum_attention(aux, self.curr_layer, self.select_crit, x)
    ...
    def forward(self, x: torch.Tensor, attns=None) -> torch.Tensor:
        ...
        if self.select_crit in ['attn', 'agls', 'gls']:
            x, idx, compl = self.prune_tokens(x, attn)
        elif self.select_crit in ['dar', 'ar', 'dmaws', 'maws', 'glsdmaws', 'glsmaws', 'mix']:
...
class MCRiT(nn.Module):
    ...
    def __init__(
        ...
        select_crit = getattr(args, 'select_crit', ['glsdmaws'])
        dropped_token_fusion = getattr(args, 'dropped_token_fusion', False)

        self.mlar = any([kw in select_crit for kw in ['dar', 'ar', 'dmaws', 'maws', 'glsdmaws', 'glsdmaws', 'mix']])

        if len(token_ratio) == 1:
            token_ratio = [token_ratio[0] for _ in range(len(reduction_loc))]

        if len(select_crit) == 1:
            select_crit = [select_crit[0] for _ in range(len(reduction_loc))]            
        ...
        assert len(select_crit) == len(reduction_loc), f"Mismatch between the reduction location ({reduction_loc}) and select criteria ({select_crit})"

        print(token_ratio, select_crit, reduction_loc)

        token_ratio_full = [1 for _ in range(depth)]
        select_crit_full = [None for _ in range(depth)]
        for idx, loc in enumerate(reduction_loc):
            token_ratio_full[loc] = token_ratio[idx]
            select_crit_full[loc] = select_crit[idx]
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                select_crit=select_crit_full[i],
                curr_layer=i,
            )
        ...
        if self.mlar:
            ...
            for name, _ in self.named_modules():
                # checks if maws or mix are part of the criterions
                if any([any([kw in crit for crit in select_crit]) for kw in ['maws', 'mix']]) and ('attn_drop' in name or 'pre_softmax' in name):
                    layers.append(name)
...
@torch.no_grad()
def _load_weights(model: MCRiT, checkpoint_path: str, prefix: str = '') -> None:
...
default_cfgs = {
    ...
    'mcrit_vit_base_patch16_224.orig_in21k': _cfg(
    ...
    'mcrit_vit_small_patch16_224.dino': _cfg(
        url='https://dl.fbaipublicfiles.com/dino/dino_deitsmall16_pretrain/dino_deitsmall16_pretrain.pth',
        hf_hub_id='timm/vit_small_patch16_224.dino',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0),
    'mcrit_vit_base_patch16_224.dino': _cfg(
    ...
    'mcrit_vit_small_patch14_reg4_dinov2.lvd142m': _cfg(
        ...
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 518, 518), crop_pct=1.0),
    'mcrit_vit_base_patch14_reg4_dinov2.lvd142m': _cfg(
    ...
    'mcrit_flexivit_base.1200ep_in1k': _cfg(
        ...
        input_size=(3, 240, 240), crop_pct=0.95),
    'mcrit_flexivit_large.1200ep_in1k': _cfg(
        ...
        input_size=(3, 240, 240), crop_pct=0.95),
    'mcrit_flexivit_base.patch16_in21k': _cfg(
...
@register_model
def mcrit_vit_tiny_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('mcrit_vit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_small_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('mcrit_vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_small_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/8)
    """
    model_args = dict(patch_size=8, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('mcrit_vit_small_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('mcrit_vit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/8) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=8, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('mcrit_vit_base_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_large_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('mcrit_vit_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_large_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('mcrit_vit_large_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_huge_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Huge model (ViT-H/14) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_args = dict(patch_size=14, embed_dim=1280, depth=32, num_heads=16)
    model = _create_vision_transformer('mcrit_vit_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch16_224_miil(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    Weights taken from: https://github.com/Alibaba-MIIL/ImageNet21K
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False)
    model = _create_vision_transformer(
        'mcrit_vit_base_patch16_224_miil', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_medium_patch16_gap_240(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'mcrit_vit_medium_patch16_gap_240', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_medium_patch16_gap_256(pretrained: bool = False, **kwargs):
    ...
    model_args = dict(
        ...
        global_pool='avg', qkv_bias=False, init_values=1e-6, fc_norm=False)
    model = _create_vision_transformer(
        'mcrit_vit_medium_patch16_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'mcrit_vit_base_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_huge_patch14_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'mcrit_vit_huge_patch14_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_giant_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'mcrit_vit_giant_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch16_clip_224(pretrained: bool = False, **kwargs):
    """ ViT-B/16 CLIP image tower
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
        'mcrit_vit_base_patch16_clip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_flexivit_small(pretrained: bool = False, **kwargs):
    """ FlexiViT-Small
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, no_embed_class=True)
    model = _create_vision_transformer('mcrit_flexivit_small', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_flexivit_base(pretrained: bool = False, **kwargs):
    """ FlexiViT-Base
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, no_embed_class=True)
    model = _create_vision_transformer('mcrit_flexivit_base', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_flexivit_large(pretrained: bool = False, **kwargs):
    """ FlexiViT-Large
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, no_embed_class=True)
    model = _create_vision_transformer('mcrit_flexivit_large', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_small_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-S/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=384, depth=12, num_heads=6, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'mcrit_vit_small_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-B/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'mcrit_vit_base_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_large_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'mcrit_vit_large_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_giant_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    ...
    model = _create_vision_transformer(
        'mcrit_vit_giant_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_small_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'mcrit_vit_small_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'mcrit_vit_base_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_large_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'mcrit_vit_large_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_giant_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    # The hidden_features of SwiGLU is calculated by:
    # hidden_features = (int(hidden_features * 2 / 3) + 7) // 8 * 8
    ...
    model = _create_vision_transformer(
        'mcrit_vit_giant_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch16_siglip_224(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'mcrit_vit_base_patch16_siglip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'mcrit_vit_base_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch16_siglip_512(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'mcrit_vit_base_patch16_siglip_512', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_large_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'mcrit_vit_large_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_medium_patch16_reg4_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8, class_token=True,
        no_embed_class=True, reg_tokens=4,
    )
    model = _create_vision_transformer(
        'mcrit_vit_medium_patch16_reg4_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_medium_patch16_reg4_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8,
        class_token=False, no_embed_class=True, reg_tokens=4, global_pool='avg',
    )
    model = _create_vision_transformer(
        'mcrit_vit_medium_patch16_reg4_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_vit_base_patch16_reg8_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False,
        no_embed_class=True, global_pool='avg', reg_tokens=8,
    )
    model = _create_vision_transformer(
        'mcrit_vit_base_patch16_reg8_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_deit_tiny_patch16_224(pretrained=False, **kwargs):
    """ DeiT-tiny model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('mcrit_deit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_deit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('mcrit_deit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_deit_base_patch16_224(pretrained=False, **kwargs):
    """ DeiT base model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('mcrit_deit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_deit3_small_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('mcrit_deit3_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_deit3_medium_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('mcrit_deit3_medium_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_deit3_base_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('mcrit_deit3_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_deit3_large_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('mcrit_deit3_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def mcrit_deit3_huge_patch14_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('mcrit_deit3_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
