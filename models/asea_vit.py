...
__all__ = ['AseaViT']  # model_registry will add each entrypoint fn to this
...
class Block(nn.Module):
    def __init__(
        ...
        self.ls1 = LayerScale(dim, init_values=ls_init_values) if ls_init_values else nn.Identity()
        ...
        self.ls2 = LayerScale(dim, init_values=ls_init_values) if ls_init_values else nn.Identity()
    ...
    def forward(self, x: torch.Tensor, H=None, W=None) -> torch.Tensor:
        ...
        if hasattr(self, 'asea') and (0 < self.keep_rate <= 1) and self.asea_pos in ['pre_block']:
            ...
            if self.class_token:
                x_cls, x = torch.split(x, [1, N-1], dim=1)

            if H and W:
                x = rearrange(x, 'b (h w) d -> b d h w', h=H, w=W)
            else:
                x = rearrange(x, 'b (h w) d -> b d h w', h=int(x.shape[1] ** 0.5))
        ...
        # need to return current h and w shape for next layers to unpack 2d shape

        if index is not None:
            return x, n_tokens, idx, H, W
        return x, n_tokens, None, H, W


class AseaViT(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        red_axis_full = [red_axis_start for _ in range(depth)]
        curr_red_axis = red_axis_start

        for loc, kr in zip(reduction_loc, token_ratio):
            token_ratio_full[loc] = kr
            red_axis_full[loc] = curr_red_axis
        ...
        self.num_clr = getattr(args, 'num_clr', 0)
    ...
    def add_clr(self, init_cls=True) -> None:
        if self.num_clr > 0:
            # self.num_clr = num_clr
            self.clr = nn.Parameter(torch.zeros(1, self.num_clr, self.embed_dim))
            nn.init.normal_(self.clr, std=1e-6)
            if not self.no_embed_class:
                print(f'Resized positional embedding from {self.pos_embed.shape}')
                embed_suffix = nn.Parameter(torch.randn(1, self.num_clr, self.embed_dim) * .02)
                self.pos_embed = nn.Parameter(torch.cat([self.pos_embed, embed_suffix], dim=1))
                print(f'To {self.pos_embed.shape}')
        if hasattr(self, 'clr') and init_cls:
           with torch.no_grad():
                cls_token_weights = repeat(self.cls_token, '1 1 d -> 1 n d', n=self.num_clr)
                self.clr.copy_(cls_token_weights)
                print(f'Loaded {self.num_clr} clr (cross-layer cache/carrier) tokens from CLS token')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            x, left_token, sample_idx, H, W = blk(x, H, W)
...
@torch.no_grad()
def _load_weights(model: AseaViT, checkpoint_path: str, prefix: str = '') -> None:
...
default_cfgs = {
    ...
    'asea_vit_base_patch16_224.orig_in21k': _cfg(
    ...
    'asea_vit_small_patch14_reg4_dinov2.lvd142m': _cfg(
        ...
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 518, 518), crop_pct=1.0),
    'asea_vit_base_patch14_reg4_dinov2.lvd142m': _cfg(
    ...
    'asea_flexivit_base.1200ep_in1k': _cfg(
        ...
        input_size=(3, 240, 240), crop_pct=0.95),
    'asea_flexivit_large.1200ep_in1k': _cfg(
        ...
        input_size=(3, 240, 240), crop_pct=0.95),
    'asea_flexivit_base.patch16_in21k': _cfg(
...
@register_model
def asea_vit_tiny_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('asea_vit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_small_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('asea_vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_small_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/8)
    """
    model_args = dict(patch_size=8, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('asea_vit_small_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('asea_vit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/8) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=8, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('asea_vit_base_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_large_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('asea_vit_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_large_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('asea_vit_large_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_huge_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Huge model (ViT-H/14) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_args = dict(patch_size=14, embed_dim=1280, depth=32, num_heads=16)
    model = _create_vision_transformer('asea_vit_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch16_224_miil(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    Weights taken from: https://github.com/Alibaba-MIIL/ImageNet21K
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False)
    model = _create_vision_transformer(
        'asea_vit_base_patch16_224_miil', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_medium_patch16_gap_240(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'asea_vit_medium_patch16_gap_240', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_medium_patch16_gap_256(pretrained: bool = False, **kwargs):
    ...
    model_args = dict(
        ...
        global_pool='avg', qkv_bias=False, init_values=1e-6, fc_norm=False)
    model = _create_vision_transformer(
        'asea_vit_medium_patch16_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'asea_vit_base_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_huge_patch14_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'asea_vit_huge_patch14_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_giant_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'asea_vit_giant_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch16_clip_224(pretrained: bool = False, **kwargs):
    """ ViT-B/16 CLIP image tower
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
        'asea_vit_base_patch16_clip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_flexivit_small(pretrained: bool = False, **kwargs):
    """ FlexiViT-Small
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, no_embed_class=True)
    model = _create_vision_transformer('asea_flexivit_small', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_flexivit_base(pretrained: bool = False, **kwargs):
    """ FlexiViT-Base
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, no_embed_class=True)
    model = _create_vision_transformer('asea_flexivit_base', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_flexivit_large(pretrained: bool = False, **kwargs):
    """ FlexiViT-Large
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, no_embed_class=True)
    model = _create_vision_transformer('asea_flexivit_large', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_small_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-S/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=384, depth=12, num_heads=6, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'asea_vit_small_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-B/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'asea_vit_base_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_large_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'asea_vit_large_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_giant_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    ...
    model = _create_vision_transformer(
        'asea_vit_giant_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_small_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'asea_vit_small_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'asea_vit_base_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_large_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'asea_vit_large_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_giant_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    # The hidden_features of SwiGLU is calculated by:
    # hidden_features = (int(hidden_features * 2 / 3) + 7) // 8 * 8
    ...
    model = _create_vision_transformer(
        'asea_vit_giant_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch16_siglip_224(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'asea_vit_base_patch16_siglip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'asea_vit_base_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch16_siglip_512(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'asea_vit_base_patch16_siglip_512', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_large_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'asea_vit_large_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_medium_patch16_reg4_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8, class_token=True,
        no_embed_class=True, reg_tokens=4,
    )
    model = _create_vision_transformer(
        'asea_vit_medium_patch16_reg4_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_medium_patch16_reg4_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8,
        class_token=False, no_embed_class=True, reg_tokens=4, global_pool='avg',
    )
    model = _create_vision_transformer(
        'asea_vit_medium_patch16_reg4_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_vit_base_patch16_reg8_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False,
        no_embed_class=True, global_pool='avg', reg_tokens=8,
    )
    model = _create_vision_transformer(
        'asea_vit_base_patch16_reg8_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_deit_tiny_patch16_224(pretrained=False, **kwargs):
    """ DeiT-tiny model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('asea_deit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_deit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('asea_deit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_deit_base_patch16_224(pretrained=False, **kwargs):
    """ DeiT base model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('asea_deit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_deit3_small_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('asea_deit3_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_deit3_medium_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('asea_deit3_medium_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_deit3_base_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('asea_deit3_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_deit3_large_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('asea_deit3_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def asea_deit3_huge_patch14_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('asea_deit3_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
